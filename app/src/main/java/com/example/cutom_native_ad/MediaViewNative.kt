...
//            mediaView?.setImageScaleType(ImageView.ScaleType.CENTER_CROP)
    ...
    nativeAd.body?.let { body ->
        adView.bodyView?.isVisible
        (adView.bodyView as? TextView?)?.text = body
    } ?: run {
        adView.bodyView?.isInvisible
    }
    nativeAd.callToAction?.let { callToAction ->
        adView.callToActionView?.isVisible
        (adView.callToActionView as? Button?)?.text = callToAction
    } ?: run {
        adView.callToActionView?.isInvisible
    }

    nativeAd.icon?.let {
        adView.iconView?.isVisible
        (adView.iconView as? ImageView?)?.setImageDrawable(it.drawable)
    } ?: run {
        adView.iconView?.isInvisible
    }

    nativeAd.price?.let { price ->
        adView.priceView?.isVisible
        (adView.priceView as? TextView?)?.text = price
    } ?: run {
        adView.priceView?.isInvisible
    }

    nativeAd.store?.let { store ->
        adView.storeView?.isVisible
        (adView.storeView as? TextView?)?.text = store
    } ?: run {
        adView.storeView?.isGone
    }


    nativeAd.starRating?.let { starRating ->
        adView.starRatingView?.isVisible
        (adView.starRatingView as? RatingBar?)?.rating = starRating.toFloat()
    } ?: run {
        adView.starRatingView?.isGone
    }

    nativeAd.advertiser?.let { advertiser ->
        adView.advertiserView?.isVisible
        (adView.advertiserView as? TextView?)?.text = advertiser
    } ?: run {
        adView.advertiserView?.isGone
    }
